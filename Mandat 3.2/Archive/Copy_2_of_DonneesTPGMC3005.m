...
imax = 50;            % Nombre d'itérations maxaximale; N.B. -> Pas bcp de différence pout
                       % tout imax > 10;
discr = 1000;           % Nombre d'élément de discrétisation de l'espace;
xmax = [0.8 1.6 3.2];      % Hauteurs du garnissage;
...
tlim = 1800;            % Temps maximum de calcul secondes;
flag = 0;
...
while flag == 0
    if cputime - tstart > tlim
        ...
        for j = 1:length(xmax)
            ...
            for  i = 1:imax
                Phase = 1;
                sol1 = pdepe(m, @pdefun, @icfun, @bcfun, x, t); % sol = u(t, x) qui est equivalent a T(t, x)
                Tinit = sol1(discr,:);  % Assigne les toutes les températures  % VOIr si ca pourrait pas être Tinit = sol(discr, dicr))
                                    % de la solution de la Phase 1
                                    % à la solution initiale pour 
                                    % la Phase 2 qui va suivre ;
                Phase = 2;
                sol2 = pdepe(m, @pdefun, @icfun, @bcfun, x, t); % sol = u(t, x) qui est equivalent a T(t, x)
                Tinit = sol2(discr,:);  % Assigne les toutes les températures 
                                    % de la solution de la Phase 2
                                    % à la solution initiale pour 
                                    % la Phase 3 qui va suivre ;
                Phase = 3;
                sol3 = pdepe(m, @pdefun, @icfun, @bcfun, x, t); % sol = u(t, x) qui est equivalent a T(t, x)
            ...
            % sa valeur actuelle dans l'aluminerie soit h = 1.6 [m] ;
